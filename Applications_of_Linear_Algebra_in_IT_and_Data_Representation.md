...
2. **TF-IDF Weighting:** $TF	ext{-}IDF(t, d, D) = TF(t, d) 	imes \log\left(rac{|D|}{|\{d \in D : t \in d\}|}ight)$
...
$$\mathbf{X} = \mathbf{U} \mathbf{\mathbf{\Sigma}} \mathbf{V}^T$$
...
$$\min_{\mathbf{P}, \mathbf{Q}} \sum_{(u,i) \in R_{known}} (R_{ui} - \mathbf{p}_u \mathbf{q}_i^T)^2 + \lambda (\|\mathbf{p}_u\|_2^2 + \|\mathbf{q}_i\|_2^2)$$
